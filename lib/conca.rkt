  ...
  (if (pair? code)
      (let ([word (car code)]
        ...
        (match word
          ...
          ['sinh         (*vm* (sinh accum) stack sequence rstack env)]
          ['cosh         (*vm* (cosh accum) stack sequence rstack env)]
          ['tanh         (*vm* (tanh accum) stack sequence rstack env)]
          ...
          ['atan2        (*vm* (atan (car stack) accum) (cdr stack) sequence rstack env)]
          ['pi           (*vm* pi (cons accum stack) sequence rstack env)]
          ['+inf         (*vm* +inf.0 (cons accum stack) sequence rstack env)]
          ['-inf         (*vm* -inf.0 (cons accum stack) sequence rstack env)]
          ['inf?         (*vm* (infinite? accum) stack sequence rstack env)]
          ['nan          (*vm* +nan.0 (cons accum stack) sequence rstack env)]
          ['-nan         (*vm* -nan.0 (cons accum stack) sequence rstack env)]
          ['nan?         (*vm* (nan? accum) stack sequence rstack env)]
